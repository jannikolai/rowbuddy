...
\hluc{HLUC-3}{Manage Routes}{Online Storage}{1}{wal}{}{%
		...
		\end{figure}
			...
			\hluc{HLUC-4}{Start rowing a route}{Route and Event Logger}{1}{jnt}{}{%
				In this use case a rowed route is recorded in the electronic logbook. The user that will start rowing a route selects a boat, the team members and a route.
			}
	
			\hluc{HLUC-5}{Stop rowing a route}{Route and Event Logger}{1}{jnt}{}{%
				In this use case a started route created in HLUC-4 is stopped to provide the functionality to return from a trip. The route is then added in the users profile.
			}
	
			\hluc{HLUC-6}{Log rowed route}{Route and Event Logger}{2}{jnt}{}{%
		...
		In this use case the program automatically create a statistic of all members on demand. The statistic contains information about the most valuable rower, most improved rower, graphical representation and best rookies. This use case also include browsing and creating statistics. The members are able to see the generated statistics.
		}

\hluc{HLUC-9}{Generate boat statistics}{Report Generator}{2}{dch}{}{
		In this use case the programm automatically create a statistic of all boats on demand. This statistic includes new boats, broken boats and boats are no longer used. The users can see this statistic. This use case includes browsing and creating statistics.
		}

\hluc{HLUC-10}{Generate individual statistics for a member}{Report Generator}{1}{dch}{}{%
		Each member has an own personal statistic about his activities. This statistic is generated on demand by the member itself. The statistic is attached to the users profile and contains information about total distance, popular routes and used boats for the current year and each year in the past.
		}
...
% online club member: club member that has entered his email address}
		...
		\hluc{HLUC-11}{Login user}{Member's Profile}{1}{gef}{}{%
...
The user can change his password. For that he has to be logged into the system. The user enters his current password and the new password twice. The user cannot use the old password any more for a login, the new password is valid now.}
		...
		\hluc{HLUC-14}{Publish route on profile}{Member's Profile}{2}{gef}{}{%
A club member can select a trip that he has participated in and can publish it on his profile site. For this trip selection he can browse his rowed trips and can search for specific trips. The visitors of his profile site can see all relevant characteristics of the trip.}

		\hluc{HLUC-15}{View other club members profile}{Member's Profile}{1}{gef}{}{%
A club member can view the profile sites of other club members. The club member sees information that a user has declared as public including the summed statistics of the user. These statistics are the number of trips, the average length of the trips and the total length of the trips. These statistics are visible for the current year and for all years. }
