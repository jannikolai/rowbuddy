...
public class StartTripPresenter implements Presenter {
	...
	private static Logger logger = Logger.getLogger(StartTripPresenter.class
			.getName());
	...
	private void bind() {
		...
		view.getAddButton().addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent arg0) {
				startTrip(new ServerRequestHandler<Void>(eventBus,
						"Fahrt starten", new ListPersonalOpenTripsEvent(), null));
			}
		});
			...
			if (route == null) {
				...
				if (tripMembers.size() == 0) {
					...
					try {
						...
						logbookService.startTrip(trip, boat.getId(),
